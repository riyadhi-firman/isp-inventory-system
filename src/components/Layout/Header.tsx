...
const Header: React.FC<HeaderProps> = ({ onToggleSidebar }) => {
  const { dashboardData, lowStockItems } = useDashboard();
  const { user, logout } = useAuth();

  const lowStockCount = lowStockItems?.length || 0;
  const pendingTransactionsCount = dashboardData?.pendingTransactions || 0;
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                ...
                <div className="p-1">
                  ...
                  <button 
                    onClick={logout}
                    className="flex items-center space-x-2 w-full p-2 text-left hover:bg-gray-50 rounded-md text-sm text-red-600"
                  >
